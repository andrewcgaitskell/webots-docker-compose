"""
PID line following controller for TPBot, ported from the real
C++/CODAL firmware (main.cpp) state machine. Uses PID control instead of
...
Two binary sensors (thresholded), same states as the firmware:
  ...
  CorrectingLeft/Right - one sensor on line - PID adjusts speed
...
K_P = 2.0      # Proportional gain - how strongly we react to error
K_I = 0.1      # Integral gain - accumulates past errors
...
class PIDLineFollower:
    ...
    def calculate_pid(self, left_black, right_black):
        """Calculate PID output based on sensor readings."""
        # Calculate error - how far we are from center line
        # Both sensors see black (1) = on line, white (0) = off line
        # Error is difference between left and right sensor readings
        error = (left_black - right_black) / 2.0
        
        # Clamp error to reasonable range to prevent PID explosion
    ...
    def drive_for_state(self, state):
        if state == STRAIGHT:
            # Both sensors on line - drive straight with slight trim
            self.left_motor.setVelocity(BASE_RAD + TRIM_RAD)
            self.right_motor.setVelocity(BASE_RAD - TRIM_RAD)
        elif state == CORRECTING_LEFT:
            # Left sensor sees line, need to turn right (left wheel slower)
            pid_output = self.calculate_pid(True, False)
            base_speed = BASE_RAD + pid_output
            self.left_motor.setVelocity(max(0.0, base_speed))
            self.right_motor.setVelocity(BASE_RAD - TRIM_RAD + pid_output)
        elif state == CORRECTING_RIGHT:
            # Right sensor sees line, need to turn left (right wheel slower)
            pid_output = self.calculate_pid(False, True)
            base_speed = BASE_RAD + pid_output
            self.left_motor.setVelocity(BASE_RAD - TRIM_RAD + pid_output)
            self.right_motor.setVelocity(max(0.0, base_speed))
        elif state == SEARCHING_LEFT:
            # Lost line, last known direction was left, turn right to find it
            self.left_motor.setVelocity(SEARCH_INNER_RAD)
            self.right_motor.setVelocity(SEARCH_OUTER_RAD)
        elif state == SEARCHING_RIGHT:
            # Lost line, last known direction was right, turn left to find it
